...
<div class="bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 min-h-screen py-0 px-0">
    <div class="max-w-3xl mx-auto">
        ...
        @php
            $assinaturaAtiva = auth()->check() ? auth()->user()->assinaturaAtiva : null;
        @endphp
        @if(auth()->check() && $assinaturaAtiva && $assinaturaAtiva->status === 'ativa')
            <div class="max-w-lg mx-auto mt-10 bg-white border border-green-200 rounded-2xl shadow-lg p-8 text-center">
                ...
                <a href="{{ route('assinatura.cancelar.form') }}" class="w-full block bg-gray-200 hover:bg-red-100 text-red-700 font-semibold py-3 rounded-lg border border-red-200 transition mt-6">Cancelar assinatura</a>
            </div>
        @else
            {{-- Conteúdo principal visível para todos (deslogado ou logado sem assinatura ativa) --}}
